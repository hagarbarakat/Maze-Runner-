...
public class DownState implements DirectionState {
  ...
  static boolean isShielded;
	...
	public void updateDirection() {
		...
		else if(startY>0&&ground[startX+1][startY]==0)
		{
			...
			Moving.getNodeByRowColumnIndex(startY, startX, pane).getChildren().add(new Pane(Player.getInstance().getImageView("down")));
		...
		else if(ground[startX+1][startY]==99) {
			maze.setElement(startX, startY, 1);
			maze.setElement(startX + 1, startY, 88);
			Pane next = Moving.getNodeByRowColumnIndex(startY, startX + 1, pane);
			...
			direction = "down";
       	  ...
       	  e.addObserver(h);
			  if(isShielded) {
            	 ...
            	 //e.setUpdate(Type.fixed);
            	  e.notifyAllObservers(Type.fixed);
            	 System.out.println("dakhlt ba3d finished");
            	  System.out.println(" Health = " + h.getHealth());

              }
			  else //if(!Armor1.isShielded()) 
			  {
				  System.out.println("hisham da arwash wa7ed fena");
			//e.setUpdate(Type.minus);
	...
	public void updateInfo(GridPane pane, Bullet b, int startX, int startY, String d) {
		DownState.pane=pane;
		this.bullet=b;
		DownState.startX=startX;
		DownState.startY=startY;
