...
public class RightState  implements DirectionState{
	...
	public void updateDirection() {
		...
		else if(startY>0&&ground[startX][startY+1]==0)
		{
			...
			Moving.getNodeByRowColumnIndex(startY, startX, pane).getChildren().add(new Pane(Player.getInstance().getImageView("right")));
		...
		else if(startY>0&&ground[startX][startY+1]==25)
		{
			...
			org.set(new Checkpoint(maze.getMaze(), Bullet.getInstance().getCounter(), h.getHealth(),s.getScore(), null ));
	...
	public void updateInfo(GridPane pane, Bullet b, int startX, int startY, String d) {
		...
		RightState.pane=pane;
		this.bullet=b;
		RightState.startX=startX;
		RightState.startY=startY;
